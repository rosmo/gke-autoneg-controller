...
func (b *BackendController) ReconcileBackends(actual, intended AutonegStatus) (err error) {
	...
	oldSvc = make(map[string]*compute.BackendService, len(removes))
	...
	for port, upsert := range upserts {
		...
		for _, d := range remove.backends {
			for i, be := range oldSvc[port].Backends {
				if d.Group == be.Group {
					copy(oldSvc[port].Backends[i:], oldSvc[port].Backends[i+1:])
					oldSvc[port].Backends = oldSvc[port].Backends[:len(oldSvc[port].Backends)-1]
					break
				}
			}
		}
